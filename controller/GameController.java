...
public class GameController {
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (!(end.isEmpty()
            ...
            return false;
        } else if (!(GridFX.adjacent(end, start))) {
            Alert error = new Alert(Alert.AlertType.WARNING);
            error.setHeaderText("Cannot be moved!");
            error.setTitle("Tile cannot be moved");
            error.showAndWait();
            return false;
        }
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            state = GameState.NEUTRAL;
            return;
        } else if (enemy.getOccupant().getOwner()
            == attacker.getOccupant().getOwner()) {
            Alert error = new Alert(Alert.AlertType.WARNING);
            error.setTitle("Cannot be attacked!");
            error.setHeaderText("This is my military!");
            error.showAndWait();
        }
